...
- **H.2 (next)**: see `BACKLOG.md`. Active items: H.2.6 (`invokesRequiredSkills` verifier check). Deferred (in backlog): H.2.7 (full pattern contracts — DOCUMENTATION-DEBT FLAG), H.2.8 (on-demand budget extensions), H.2.9 (`chaos-test --pattern <name>` runner).
